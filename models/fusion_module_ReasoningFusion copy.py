...
class ReasoningFusion(nn.Module):
    def __init__(self, visual_dim, text_dim):
        ...
        self.attn = nn.MultiheadAttention(visual_dim, num_heads=4, batch_first=True)
    ...
    def forward(self, visual_feat, text_feat):
        ...
        return shortcut + self.alpha * v_reasoned
...
class CoordGatedFusion(nn.Module):
    ...
    def forward(self, x, t_vec):
        ...
        coords = torch.stack([grid_x, grid_y]).unsqueeze(0).repeat(b, 1, 1, 1)
        
        x = self.coord_conv(torch.cat([x, coords], dim=1))
        
        # 通道门控
        gate = self.gate_gen(t_vec).view(b, c, 1, 1)
        return x * gate
...
class DeepFusionPAFPN(nn.Module):
    def __init__(self, in_channels=[128, 256, 512], text_dim=1024):
        ...
        self.fuse_p4 = CoordGatedFusion(c4, text_dim)
