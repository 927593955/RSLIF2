...
class SpatialRelationReasoner(nn.Module):
    """Language-conditioned relation reasoning block inspired by SpatialVLM-style spatial priors."""
    def __init__(self, channels, text_dim, reduction=4):
        super().__init__()
        hidden = max(channels // reduction, 32)
        self.visual_proj = nn.Conv2d(channels, hidden, kernel_size=1, bias=False)
        self.coord_proj = nn.Conv2d(2, hidden, kernel_size=1, bias=False)
        self.text_proj = nn.Sequential(
        ...
        self.out_proj = nn.Conv2d(hidden, channels, kernel_size=1, bias=False)
        self.out_norm = nn.BatchNorm2d(channels)
        self.gain = nn.Parameter(torch.tensor(0.1))
        self._coord_cache = {}

    def _coords(self, h, w, x):
        key = (h, w, x.device.type, x.device.index, x.dtype)
        if key not in self._coord_cache:
            ...
            self._coord_cache[key] = torch.stack([gx, gy], dim=0).unsqueeze(0)
        return self._coord_cache[key]

    def forward(self, feat, text_vec):
        b, _, h, w = feat.shape
        coords = self._coords(h, w, feat).expand(b, -1, -1, -1)
        tokens = self.visual_proj(feat) + self.coord_proj(coords)

        q = self.text_proj(text_vec).unsqueeze(-1).unsqueeze(-1)
        gate = torch.sigmoid((tokens * q).sum(dim=1, keepdim=True) / (tokens.size(1) ** 0.5))

        reasoned = self.local_relation(tokens)
        reasoned = reasoned * gate
        out = self.out_norm(self.out_proj(reasoned))
        return feat + torch.tanh(self.gain) * out
...
class DeepFusionPAFPN(nn.Module):
    def __init__(self, in_channels=[128, 256, 512], text_dim=1024):
        ...
        # 先做关系建模（language-conditioned relation reasoning）
        self.spatial_relation = nn.ModuleDict({
            'p3': SpatialRelationReasoner(c3, text_dim),
            'p4': SpatialRelationReasoner(c4, text_dim),
            'p5': SpatialRelationReasoner(c5, text_dim),
        })
    ...
    def forward(self, raw_p3, raw_p4, raw_p5, sem_text, spa_text, attr_text, mask):
        ...
        # 1.1 关系先验注入：在 FPN 前先对多尺度视觉特征做语言条件关系推理
        raw_p3 = self.spatial_relation['p3'](raw_p3, spa_pyr['p3'])
        raw_p4 = self.spatial_relation['p4'](raw_p4, spa_pyr['p4'])
        raw_p5 = self.spatial_relation['p5'](raw_p5, spa_pyr['p5'])
...
class RemoteSensingVLM(nn.Module):
    def __init__(self, cfg):
        ...
        self.spa_film = nn.ModuleList([
            nn.Linear(self.text_dim, c * 2) for c in self.backbone.out_channels
        ])

        # 语义向量对分类 logits 的先验偏置（增强类别敏感性）
        self.cls_text_bias = nn.Linear(self.text_dim, nc)
        self.cls_bias_scale = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, imgs, texts):
        ...
        for i, feat in enumerate(fused_feats):
            gb = self.spa_film[i](relation_vec)
            gamma, beta = torch.chunk(gb, 2, dim=1)
            gamma = torch.tanh(gamma).unsqueeze(-1).unsqueeze(-1)
            beta = beta.unsqueeze(-1).unsqueeze(-1)
            mod_feats.append(feat * (1.0 + gamma) + beta)
