...
class BertAttention(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask, return_att=False, query_states=None, relative_pos=None, rel_embeddings=None, branch_type="normal", layer_id=0, total_layers=6):
    output = self.self(hidden_states, attention_mask, return_att, query_states=query_states, relative_pos=relative_pos, rel_embeddings=rel_embeddings,\
                        branch_type=branch_type, layer_id=layer_id, total_layers=total_layers)
...
class BertEncoder(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, return_att=False, query_states = None, relative_pos=None, branch_type="normal", layer_id=0, total_layers=6):
    if attention_mask.dim()<=2:
