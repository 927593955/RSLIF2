...
class DisentangledSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, return_att=False, query_states=None, relative_pos=None, rel_embeddings=None,
                branch_type="normal", layer_id=0, total_layers=6): # <--- 新增参数
        """
        branch_type: "normal" | "semantic" | "spatial"
        layer_id: 当前分支内的相对层号 (0 ~ total_layers-1)
        total_layers: 当前分支的总层数
        """
        ...
        w_content = 1.0 # 控制 C-to-C 的权重
        w_pos = 1.0     # 控制 Position (rel_att) 的权重
        ratio = (layer_id+1) / max(total_layers, 1)

        if branch_type == "semantic":
            # 语义分支策略：随着层数加深，弱化位置信息，专注于内容匹配
            # Position Weight: 1.0 -> 0.1 (抑制90%)
            w_pos = max(0.0, 1.0 - 0.9 * ratio)
            # Content Weight:  1.0 -> 1.1 (增强10%)
            w_content = 1.0 + 0.1 * ratio

        elif branch_type == "spatial":
            # 空间分支策略：随着层数加深，强行依赖位置信息，弱化内容匹配
            # Position Weight: 1.0 -> 1.5 (增强50%)
            w_pos = 1.0 + 0.5 * ratio
            # Content Weight: 1.0 -> 0.5 (抑制50%)
        ...
        if self.relative_attention:
            ...
            # 计算 Position 相关分数 (C-to-P + P-to-C + P-to-P)
